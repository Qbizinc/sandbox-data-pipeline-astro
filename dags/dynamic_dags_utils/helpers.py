...
def lag_pandas(df, grp, n_lags):
    ...
    lag_dict = {
        f"sales-lag{lag}": data_grp_pd.drop(["quarter"], axis=1)
        .groupby(level=grp)
        .shift(lag)
        .rename(columns=lambda x: x + f"lag{lag}")
        for lag in range(1, n_lags)
    }

    lagged_pd = data_grp_pd.assign(**lag_dict).reset_index().dropna()

    return lagged_pd


def train_model(
    item, df, grp, model_dict, categories_dict, model_path="dags/data/models"
):
    import pandas as pd
    ...
    tmp_dict = {
        f"{col}": f"{val}" for val, col in zip(value_list, categories_dict["columns"])
    }
...
def apply_model(item, df, grp, categories_dict, model_path="dags/data/models"):
    import pandas as pd
    ...
    model = joblib.load(
        model, "{}/{}.joblib".format(model_path, "-".join(value_list)), compress=1
    )
...
def save_data(X, y, file_name):
    ...
    data_pd = pd.DataFrame(X, columns=X.columns).reset_index()
    data_pd["y"] = y
